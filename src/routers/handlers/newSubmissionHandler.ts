...
export class NewSubmissionHandler {
    ...
    // todo(1): implement this fully with the web app, when submitting an extension request // NOSONAR
    public async handleNewSubmission (req: Request, res: Response): Promise<string> {
        ...
        // todo(2): ensure we have server side validation to not allow MORE than 2 verification requests // NOSONAR
        ...
        if (this.isErrorResponse(resource)) {
            // todo(any): proper error page i.e extensions already submitted/can't submit more // NOSONAR
            // todo(any): 500, server error or something went wrong page if server error? // NOSONAR
            ...
            // todo(any): route to generic extension successful page, or have a page per extension req? // NOSONAR
    ...
    public async createNewSubmission (request: Request, transaction: Transaction): Promise<Resource<PscExtensions> | ApiErrorResponse> {
        ...
        // todo(any): better enums/keys for extension reasons // NOSONAR
