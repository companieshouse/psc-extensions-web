...
describe("PageNotFoundHandler", () => {
    ...
    describe("getViewData", () => {
        it("should return baseViewData with page-not-found template name and locale info", async () => {
            const handler = new TestablePageNotFoundHandler();
            ...
            expect(viewData.templateName).toBe(PREFIXED_URLS.PAGE_NOT_FOUND.slice(1));
    ...
    describe("executeGet", () => {
        it("should return template path and viewData", async () => {
            ...
            expect(viewModel.templatePath).toBe(ROUTER_VIEWS_FOLDER_PATH + "/error/page-not-found");
            expect(viewModel.viewData.templateName).toBe(PREFIXED_URLS.PAGE_NOT_FOUND.slice(1));
        });
    });
